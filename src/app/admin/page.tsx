...
function getLv(xp: number) {
  let r = ALL_LV[0]
  for (let i = ALL_LV.length - 1; i >= 0; i--) {
    if (xp >= ALL_LV[i].xp) { r = ALL_LV[i]; break }
  }
  return r
}
...
export default function AdminPage() {
  ...
  async function fetchData() {
    ...
    const [{ data: u }, { data: s }] = await Promise.all([
      supabase.from('silica_training_users').select('*').order('total_xp', { ascending: false }),
      supabase.from('silica_training_sessions').select('*').order('trained_at', { ascending: false }).limit(100),
    ])
    if (u) setUsers(u)
    if (s) setSessions(s)
    setLoading(false)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a1a', color: '#fff', fontFamily: 'sans-serif', padding: '24px' }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <h1 style={{ fontSize: 24, fontWeight: 'bold', marginBottom: 8 }}>🛡️ 管理画面 — ケイソくんトレーニング</h1>
        <p style={{ color: '#888', marginBottom: 24 }}>ユーザー数: {users.length} 人 ／ セッション数: {sessions.length} 件</p>
          ...
          {(['users', 'sessions'] as const).map(tab => (
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                fontWeight: 'bold',
                background: activeTab === tab ? '#6c3483' : '#1a1a2e',
                color: '#fff',
              }}
            >
              {tab === 'users' ? '👥 ユーザー一覧' : '📊 セッション履歴'}
            </button>
          ))}
          <button
            onClick={fetchData}
            style={{ marginLeft: 'auto', padding: '8px 16px', borderRadius: 8, border: 'none', cursor: 'pointer', background: '#1a3a2e', color: '#2ecc71' }}
          >
            🔄 更新
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {users.map((u, i) => {
              const lv = getLv(u.total_xp)
              return (
                <div key={u.user_name} style={{ background: '#1a1a2e', borderRadius: 12, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16 }}>
                  <span style={{ fontSize: 20, fontWeight: 'bold', color: '#555', width: 28 }}>#{i + 1}</span>
                  <span style={{ fontSize: 28 }}>{lv.emoji}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 'bold', fontSize: 16 }}>{u.user_name}</div>
                    <div style={{ color: '#888', fontSize: 13 }}>
                      Lv.{lv.lv} {lv.name} ／ セッション {u.session_count} 回
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 18, fontWeight: 'bold', color: '#ffd166' }}>{u.total_xp.toLocaleString()} XP</div>
                    <div style={{ fontSize: 12, padding: '2px 8px', borderRadius: 9999, background: rankColor[lv.rank] || '#555', display: 'inline-block', marginTop: 4 }}>
                      {lv.rank}
                    </div>
                  </div>
                </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead>
                <tr style={{ color: '#888', borderBottom: '1px solid #333' }}>
                  <th style={{ padding: '8px 12px', textAlign: 'left' }}>日時</th>
                  <th style={{ padding: '8px 12px', textAlign: 'left' }}>ユーザー</th>
                  <th style={{ padding: '8px 12px', textAlign: 'center' }}>スコア</th>
                  <th style={{ padding: '8px 12px', textAlign: 'center' }}>獲得XP</th>
                </tr>
              </thead>
              <tbody>
                {sessions.map(s => (
                  <tr key={s.id} style={{ borderBottom: '1px solid #1a1a2e' }}>
                    <td style={{ padding: '8px 12px', color: '#888' }}>
                      {new Date(s.trained_at).toLocaleString('ja-JP')}
                    </td>
                    <td style={{ padding: '8px 12px', fontWeight: 'bold' }}>{s.user_name}</td>
                    <td style={{ padding: '8px 12px', textAlign: 'center' }}>
                      {s.score}/{s.total}
                      <span style={{ color: '#888', marginLeft: 4 }}>({Math.round(s.score / s.total * 100)}%)</span>
                    </td>
                    <td style={{ padding: '8px 12px', textAlign: 'center', color: '#ffd166', fontWeight: 'bold' }}>
                      +{s.xp_gained} XP
                    </td>
                  </tr>
                ))}
