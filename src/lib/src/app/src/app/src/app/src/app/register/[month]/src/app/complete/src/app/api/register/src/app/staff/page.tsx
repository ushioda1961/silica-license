...
export default function StaffPage() {
  ...
  useEffect(() => {
    if (!authed) return
    setLoading(true)
    supabase.from('applicants').select('*').eq('desired_month', tab).eq('status','confirmed').order('created_at',{ascending:false})
    ...
    <div className="min-h-screen bg-slate-800 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-xl p-10 w-full max-w-sm mx-6">
        ...
        <div className="space-y-4">
          <div><label className="block text-sm font-semibold text-slate-700 mb-1">ログインID</label>
            <input className="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm bg-blue-50 focus:outline-none focus:ring-2 focus:ring-teal-500" value={id} onChange={e=>setId(e.target.value)} placeholder="ID" /></div>
          <div><label className="block text-sm font-semibold text-slate-700 mb-1">パスワード</label>
            <input className="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm bg-blue-50 focus:outline-none focus:ring-2 focus:ring-teal-500" type="password" value={pw} onChange={e=>setPw(e.target.value)} placeholder="パスワード" onKeyDown={e=>e.key==='Enter'&&login()} /></div>
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-6xl mx-auto py-8 px-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          {loading ? <div className="p-12 text-center text-slate-400">読み込み中...</div> : rows.length===0 ? <div className="p-12 text-center text-slate-400">申込みはありません</div> : (
                ...
                <tbody className="divide-y divide-slate-100">
                  {rows.map(r=>(
                    <tr key={r.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3"><div className="font-medium text-slate-800">{r.name}</div><div className="text-xs text-slate-400">{r.furigana}</div></td>
